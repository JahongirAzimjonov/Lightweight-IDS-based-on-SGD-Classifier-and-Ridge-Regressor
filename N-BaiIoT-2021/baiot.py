...
def backward_sequential_feature_selection(X, y, feature_select_model, number_of_features_to_select):
    ...
    sfs_backward = SequentialFeatureSelector(
        feature_select_model, n_features_to_select=number_of_features_to_select, direction="backward",).fit(X, y)
        # feature_select_model, n_features_to_select="auto", direction="backward",).fit(X, y)
    toc_bwd = time()
    # print(
...
if __name__ == "__main__":
    ...
    # df = df.iloc[50000:55000]
    dataset_text = "Dataset Class Distribution Imbalance \nTest for the N-BaIoT Dataset"
    ...
    # bw_features = selected_feature_names_with_importance
    sfm_imp_coef = SelectFromModel(estimator = fs_model, threshold=threshold).fit(X, y)
    imp_coef_features = feature_names[sfm_imp_coef.get_support()]
    backward_selected_features = backward_sequential_feature_selection(X=X[imp_coef_features], 
                                                                    #    X=X,
                                                                       y=y, 
                                                                       feature_select_model = fs_model, 
                                                                       number_of_features_to_select=number_of_features_to_select
